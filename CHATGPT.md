...
- Notionへ新しい記録を作成したり既存の記録へ追記・編集したりする前に, 同じデータベースまたは親ページ内にある同種の既存記録を確認し, 記載の書き方を踏襲すること.
- 対象日付の既存記録があればそれを参照すること. 対象日付に記録がなければ, 他の日付の同種の既存記録を参照すること. 複数ある場合は用途が一致し, 日付が近い記録を優先すること.
- タイトルや日付の表記, プロパティ, 見出し, 項目の順序, 箇条書き, 改行, 文体などを既存の形式に合わせること. 独自のテンプレートや書式へ変更しないこと.
- 既存記録から流用するのは書き方だけとし, 他の日付の内容や数値, 事実をそのまま転記しないこと. 新しい記録には対象日付について確認できた情報だけを書くこと.
- 参照できる既存記録が一件もない場合は, userの指定と既存のデータベース構造に従い, 未確認の情報や形式を推測して補わないこと.
